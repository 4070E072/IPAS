...
二.
    Smurf Attack
      ...
      攻擊手法:
              1.攻擊者偽冒受害者IP發送大量回應請求(Echo Request)給B網段的廣播(Broadcast)IP位址 (B.255或B.0) ->  B網段所有電腦收到回應請求(Echo Request)封包，都回覆(Echo Reply)封包給Victim -> 導致Victim端頻寬被大量的回復(Echo Reply)封包所占用
